...
A skill `backlog-import` é o workflow agent-led V1 JSON → V2: primeiro lê e valida o JSON inteiro, roda doctor e apresenta uma proposta completa sem mutação; somente após confirmação humana explícita executa o plano com store init/backlog create|bind/item add/transition|move pelo caminho exato do backlogctl. Ela não é um comando CLI, não modifica o legado e emite mapa `legacy_id → v2_id`. A migração cross-item não é atômica; falha com IDs já criados e relatório para retomada.
